...
## check out "http://www.astrophysik.uni-kiel.de/index.php?option=com_content&view=article&id=80%3Agnuplot&Itemid=88&lang=de"
   ...
   SOL=$2$i.gnu
  gnuplot -e "set terminal jpeg; set mxtics 2; set mytics 2;  set grid xtics ytics mxtics mytics; plot '$SOL' using 1:2 w l lw 2, '$SOL' using 1:3 w l lw 2" > pic_$ext.${TYPE}
done  
