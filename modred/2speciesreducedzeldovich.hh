#ifndef TWO_SPECIES_TEST_REDUCED_ZELDOVICH_MECHANISM_BY_HAND_HH
#define TWO_SPECIES_TEST_REDUCED_ZELDOVICH_MECHANISM_BY_HAND_HH
...
namespace Adonis{
  ...
  template<class T>
  class TwoSpeciesReducedZeldovich{
  public:
    ...
    typedef Zeldovich<DType> FullModelType;
    typedef JacS<DType,Zeldovich,ExprTmpl::MyVec> JacobianType;


    //n = #red + 1 (temp) 
    TwoSpeciesReducedZeldovich(SizeType n = 2+1):maxIt_(13),     count_(1),
				       maxNewt_(5),
				       tol_(1.e-10),
				       newtTol_(1.e-08),
				       fullode_(8),
				       fulldim_(8),rhs_(n),zM_(8), zM0_(8), C_(7*1), b_(1), residual_(1), z_nu_(8), g_(1), defect_(7), G_(8),Gprime_(8*8),up_(8), low_(8),zMchem_(7) {
    ...
    VType& operator()(const X& red){
      ...
      //only two species serve as RPV, N2 and O2
      zM_[N2] = red[0];
      zM_[O2] = red[1];

      zM_[TMP] = red[2]; //temperature
	...
	wbar_ += zM_[k]/W_[k];
      ...
      dotomega_[0] = -k1*Conc_[O]*Conc_[N2] + km1*Conc_[NO]*Conc_[N] 
                     - km2*Conc_[NO]*Conc_[O] + k3*Conc_[N]*Conc_[OH] 
		       - km3*Conc_[NO]*Conc_[H];
      //O2
      dotomega_[1] = -k2*Conc_[N]*Conc_[O2] + km2*Conc_[NO]*Conc_[O];

      //!SPECIES in mass fractions
      //! compute in mass fractions, i.e. \f$1/\rho\dot{\omega}W_k\f$
      rhs_[0] = invrho*(dotomega_[0]*W_[N2]);
      rhs_[1]  = invrho*(dotomega_[1]*W_[O2]);
	...
	heat_ = enthalpy(zM_[TMP],&coef_N2_[0])*dotomega_[0] +
	  enthalpy(zM_[TMP],&coef_O2_[0])*dotomega_[1];
      }
      else{
	...
	heat_ = enthalpy(zM_[TMP],&coef_O_[0])*fullode_.dot_omega()[O] +
	  ...
	  enthalpy(zM_[TMP],&coef_NO_[0])*fullode_.dot_omega()[NO] +
	  enthalpy(zM_[TMP],&coef_N_[0])*fullode_.dot_omega()[N]   +
	  enthalpy(zM_[TMP],&coef_O2_[0])*dotomega_[1] + //fullode_.dot_omega()[O2]   +
	  enthalpy(zM_[TMP],&coef_OH_[0])*fullode_.dot_omega()[OH] +
	  enthalpy(zM_[TMP],&coef_H_[0])*fullode_.dot_omega()[H];
      }


      rhs_[2] = -invrho*1./cp_*heat_; //mind the minus here!
