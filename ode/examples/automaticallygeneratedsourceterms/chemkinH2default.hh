#ifndef H_2_OXIDATION_DEFAULT_CHEMKIN_MECHANISM_HH
#define H_2_OXIDATION_DEFAULT_CHEMKIN_MECHANISM_HH
...
namespace Adonis{
  ...
  public:
    ...
    enum{code = 91};  //! mechanism signature
    ...
    ChemkinH2Default(size_t dim = 0, const T& rho = 1.):rhs_(dim),conc_(DataType::nspec),y_(DataType::nspec+1),rho_(rho),qheatReac_(T()),cp_(T()){
      if(dim > 0){
    ...
    size_t dim() const {return DataType::nspec+1;} //# of species + temperature
    size_t domain_dim() const {return DataType::nspec+1;}
    ...
    VType& operator()(const E& u){
      ...
      T temperature = y_[DataType::nspec];
      ...
      y_[DataType::N2] = 1. - (y_[DataType::O]+y_[DataType::O2]+y_[DataType::H]+y_[DataType::OH]+y_[DataType::H2]+y_[DataType::HO2]+y_[DataType::H2O2]+y_[DataType::H2O]);   //obeys mass conservation
