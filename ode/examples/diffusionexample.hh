#ifndef DIFFUSION_TREATED_BY_METHOD_OF_LINES_HH
#define DIFFUSION_TREATED_BY_METHOD_OF_LINES_HH
...
namespace Adonis{
   ...
   */
  template<class T>
  class DiffusionDevidedDifferences{
  private:
    T a_,
      b_,
      c_,
  ...
  public:
    ...
    enum{
      numOfSpecies = ConstantType::numberOfSpecies,
      space = ConstantType::spacePoints,            //number of space points
      domainDim = numOfSpecies*space
    };

    DiffusionDevidedDifferences(size_t dim=0):rhs_(dim){
	...
	rhs_[i] = c_/eps_*z1_z2 - d_*y[i] + D1_*(y[i+1] - 2* y[i] + y[i-1])/(ntimes<2>(h));
